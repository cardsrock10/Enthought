...
(NaN). For that, it will be valuable to look at NumPy's function that
contain nan in their names. Apply that function to the timeseries using a
window of 20. Plot the result to compare with the curve given by the regular
rolling(...).mean(). Did that help?

5. The reason it didn't help is that before the custom function is
applied, the presence of nan values is tested by pandas' rolling functions
(rolling(...).mean() and rolling(...).apply). The solution resides in the use
of its min_periods keyword argument. Use it to compute the rolling average
...
# 2.
ts20 = ts.rolling(20).mean()
ts20.plot(label="RA - 20")

ts40 = ts.rolling(40).mean()
ts40.plot(label="RA - 40")

ts80 = ts.rolling(80).mean()
...
ts20_withoutNan = ts.rolling(20).apply(np.nanmean)
ts20_withoutNan.plot(label="RA - 20 cleaned?")
# We see that didn't do the trick.

# 5.
ts20_withoutNan2 = ts.rolling(20, min_periods=19).mean()
